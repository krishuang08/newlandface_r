#添加了home参数的函数：initializeFolder，detect_face，align_face，preprocess_face
...
import keras
import bz2
from newlandface_r.commons import distance
...
def initializeFolder(home):
	
	# home = str(Path.home())
	#home = 'D:/newlandface'
	if not os.path.exists(home+"/.newlandface"):
		os.mkdir(home+"/.newlandface")
		print("Directory ",home,"/.newlandface created")
	
	if not os.path.exists(home+"/weights"):
		os.mkdir(home+"/weights")
		print("Directory ",home,"/weights created")
...
def detect_face(img,home, detector_backend = 'opencv', grayscale = False, enforce_detection = True):#这里插入home参数
	...
	if detector_backend == 'opencv':
		...
		#---------------------------
		#check required ssd model exists in the home/weights folder
		#model structure
...
def align_face(img,home, detector_backend = 'opencv'):
	...
	if (detector_backend == 'opencv') or (detector_backend == 'ssd'):
		...
		#check required file exists in the home/weights folder
		if os.path.isfile(home+'/weights/shape_predictor_68_face_landmarks.dat') != True:	
		...
		#------------------------------
		import dlib #this is not a must dependency in newlandface
		detector = dlib.get_frontal_face_detector()
		sp = dlib.shape_predictor(home+"/weights/shape_predictor_68_face_landmarks.dat")
		detections = detector(img, 1)	
		if len(detections) > 0:
...
def preprocess_face(img,home, target_size=(224, 224), grayscale = False, enforce_detection = True, detector_backend = 'opencv'):
	#img might be path, base64 or numpy array. Convert it to numpy whatever it is.
	img = load_image(img)
	base_img = img.copy()
	img = detect_face(img,home,detector_backend = detector_backend, grayscale = grayscale, enforce_detection = enforce_detection)
	#--------------------------
	if img.shape[0] > 0 and img.shape[1] > 0:
		img = align_face(img ,home, detector_backend = detector_backend)
	else:	
		if enforce_detection == True:
...
def allocateMemory():
	...
	required_memory = 10000 #All newlandface models require 9016 MiB
	...
	if df.shape[0] > 0: #has gpu
		if df.iloc[0].available_memories_in_mb > required_memory:
			...
			print("newlandface will run on GPU (gpu_", my_gpu,")")
		else:
			...
			print("newlandface will run on CPU")
	else:
		print("newlandface will run on CPU")
