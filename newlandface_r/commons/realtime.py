...
from newlandface_r.basemodels import VGGFace, OpenFace, Facenet, FbDeepFace, DeepID
from newlandface_r.extendedmodels import Age, Gender, Race, Emotion
from newlandface_r.commons import functions, realtime, distance as dst

def analysis(db_path, model_name, distance_metric, home,enable_face_analysis = True):#插入了home参数
	...
	if len(employees) > 0:
		if model_name == 'VGG-Face':
			...
			from newlandface.basemodels.DlibResNet import DlibResNet
			model = DlibResNet()
			input_shape = (150, 150)
	...
	for index in pbar:
		...
		embedding = []
		img = functions.preprocess_face(employee,home, target_size = (input_shape_y, input_shape_x), enforce_detection = False)
				...
				if freezed_frame == 0:
					...
					for detected_face in detected_faces_final:
						...
						if enable_face_analysis == True:
							...
							gray_img = functions.preprocess_face(custom_face, home,target_size = (48, 48), grayscale = True, enforce_detection = False)
							...
							face_224 = functions.preprocess_face(custom_face, home,target_size = (224, 224), grayscale = False, enforce_detection = False)
						...
						custom_face = functions.preprocess_face(custom_face, home,target_size = (input_shape_y, input_shape_x), enforce_detection = False)
