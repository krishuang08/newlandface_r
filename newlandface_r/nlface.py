...
from newlandface_r.commons import functions
from newlandface_r.extendedmodels import Age, Gender, Race, Emotion
import cv2
import json
...
def load_model(home,actions=[]):  # 加了第二个参数home,手动指定home,注意我在这里把actions放到后面了
    #global HOME
    #HOME=home
    functions.initializeFolder(home)#load_model后得到用户指定home再initializeFolder
    global predictor
    global detector
    global ssd_detector
    ...
    predictor = dlib.shape_predictor(
        home+"/weights/shape_predictor_68_face_landmarks.dat")
...
# if a specific target is not passed, then find them all
    ...
    if 'emotion' in actions:
        emotion_model = Emotion.loadModel(home)

    if 'age' in actions:
        age_model = Age.loadModel(home)

    if 'gender' in actions:
        gender_model = Gender.loadModel(home)

    if 'race' in actions:
        race_model = Race.loadModel(home)
...
def detect_face(image, detector_backend='ssd'):
    ...
    if detector_backend == 'ssd':
        ...
        detections_df['left'] = (detections_df['left']
                                 * 300*aspect_ratio_x).astype(int)
        detections_df['bottom'] = (
            detections_df['bottom'] * 300*aspect_ratio_y).astype(int)
        detections_df['right'] = (
            detections_df['right'] * 300*aspect_ratio_x).astype(int)
        detections_df['top'] = (detections_df['top'] *
                                300*aspect_ratio_y).astype(int)
        rects = []
        for index, facerect in detections_df.iterrows():
            rect = dlib.rectangle(int(facerect['left']), int(
                facerect['top']), int(facerect['right']), int(facerect['bottom']))
            rects.append(rect)
    ...
    if detector_backend == 'opencv':
        ...
        try:
            ...
            for i, facerect in enumerate(facerects):
                rect = dlib.rectangle(
                    facerect[0], facerect[1], facerect[0]+facerect[2], facerect[1]+facerect[3])
                rects.append(rect)
        except:
            pass
...
def show_face_attr(image, rect, attribute=None, actions=[]):
    ...
    if attribute is not None and len(actions) != 0:
        ...
        for action in actions:
            txtPrint = txtPrint + str(action) + ":" + \
                str(attribute[action]) + " "
        cv2.putText(image, str(txtPrint), (left, bottom),
...
def align_face(img, rect, grayscale=False, target_size=(224, 224), detector_backend='opencv'):
    ...
    if (detector_backend == 'opencv') or (detector_backend == 'ssd'):
        ...
        detected_face_gray = cv2.cvtColor(cropImg, cv2.COLOR_BGR2GRAY)
...
def analyze(img, rect, actions=[]):
    ...
    pbar = tqdm(range(0, len(actions)),
                desc='Finding actions', disable=disable_option)
    action_idx = 0
    ...
    for index in pbar:
        ...
        if action == 'emotion':
            ...
            imgcrop = align_face(img, rect, grayscale=True,
                                 target_size=(48, 48))
            ...
            if img_224 is None:
                img_224 = align_face(
                    img, rect, grayscale=False, target_size=(224, 224))
...
#functions.initializeFolder(HOME)
